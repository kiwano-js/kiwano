import { clone, compact, flatten, last } from 'es-toolkit/compat'
...
import {
    ...
    ForbiddenError,
    InvalidInputError, type Optional,
...
import {
    ...
    getModelSelectQuery,
    hooksExecutor,
    type ModelMutationResolverHooks,
    throwModelNotFound
} from "./common";
        ...
        try {
            ...
            await options.dataSource.transaction(async transaction => {
                ...
                if(hooks?.$restoreModel){
                    ...
                    const restoreQueryBuilder: SoftDeleteQueryBuilder<ModelType> = transactionRepository.createQueryBuilder().restore();
